...
function parliamentwatch_page_alter(&$page) {
  if (isset($page['content']['system_main'])) {
    ...
    $is_blog_or_petition_or_dialogue_page = menu_get_object('node') ? in_array(menu_get_object('node')->type, ['blogpost', 'dialogue', 'pw_petition']) : FALSE;
    $is_profile_page = in_array(menu_get_item()['page_callback'], ['user_view_page', 'user_revision_show']);

    if (!$has_container && !$has_filters && !$is_blog_or_petition_or_dialogue_page && !$is_profile_page) {
